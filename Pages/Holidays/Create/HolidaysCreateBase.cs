...
namespace RAS823_MC_CiudadMunicipal_FrontEnd.Pages.Holidays.Create
{
    public class HolidaysCreateBase : ComponentBase
    {
        [Inject]
        ...
        public async Task HandleFormValid()
        {
            ...
            var response = await _holidayService.PostHoliday(holidayDto);
            if (response != null && response.response.Success)
            {
                ...
                holidayManagementDto = new HolidayManagementDto();
            }
            else
            {
                _spinnerService.Hide();
